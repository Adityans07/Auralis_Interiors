...
export function AuthLayout({ title, subtitle, children, footer }: AuthLayoutProps) {
  return (
    <div className="relative min-h-[calc(100vh-5rem)] overflow-hidden bg-void">
      ...
      <div className="container-wide relative grid min-h-[calc(100vh-5rem)] items-center gap-10 py-12 lg:grid-cols-2 lg:gap-16">
        ...
        >
          <div className="glass rounded-4xl p-8 shadow-glow sm:p-10">
        ...
        <div className="relative hidden h-full min-h-[32rem] overflow-hidden rounded-4xl lg:block">
          ...
          <div className="absolute inset-0 flex flex-col justify-end p-10 text-foreground">
            <span className="inline-flex w-fit items-center gap-2 rounded-full border border-white/20 bg-base/10 px-4 py-1.5 text-xs font-medium uppercase tracking-[0.2em] text-gold-light backdrop-blur">
            ...
            <ul className="mt-6 space-y-3">
              {TRUST.map(({ icon: Icon, text }) => (
                <li key={text} className="flex items-center gap-3 text-sm text-muted-foreground/85">
                  <span className="flex h-9 w-9 items-center justify-center rounded-full bg-base/10">
